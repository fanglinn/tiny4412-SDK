/*
 * it7260_ts.c driver for tiny4412 on linux-4.19.27
*/
...
*/
static int it7260_identify(struct i2c_client *client)
{
...
static int it7260_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ret = it7260_identify(client);
	if (ret) {
		printk("cannot identify the touch screen\n");
		goto err0;
	}

	pr_info("it7260 identified.\n");
	...
	ret = request_irq(priv->irq, it7260_ts_isr, IRQF_TRIGGER_LOW,
						client->name, priv);
	if (ret) {
...
static const struct i2c_device_id it7260_ts_id_table[] =
{
    { "it7260_ts", 0 },
    {}
};
